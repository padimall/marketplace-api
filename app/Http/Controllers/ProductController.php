...
class ProductController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        $ratings = DB::table('invoices_product_ratings')
                    ...
                    ->join(DB::raw("(SELECT COUNT(IPR.id) AS total_ratings,SUM(IPR.star) AS total_star FROM invoices_product_ratings AS IPR INNER JOIN invoices_products AS IP ON IPR.invoice_product_id = IP.product_id WHERE IP.id = '$product_id') AS rating_sumarry",[$request['target_id']]),DB::raw('1'),'=',DB::raw('1'))
